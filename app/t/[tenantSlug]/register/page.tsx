  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div>
        ...
        <div style={{ position: "relative" }}>
          <input autoComplete="new-password" className={`auth-input${passwordMismatch ? " auth-input--error" : ""}`} onChange={(e) => setPasswordConfirm(e.target.value)} placeholder="Zopakujte heslo" required type={showPasswordConfirm ? "text" : "password"} value={passwordConfirm} style={{ paddingRight: "2.5rem" }} />
      ...
      <div>
        ...
        <input className="auth-input" onChange={(e) => setJoinCode(e.target.value)} placeholder="Kód od správce kantýny" required type="text" value={joinCode} autoComplete="off" />
      </div>
      ...
      <div style={{ minHeight: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem 1rem" }}>
        <div className="glass scale-in" style={{ width: "100%", maxWidth: 420, borderRadius: 24, padding: "2rem", position: "relative", zIndex: 10 }}>
          ...
          <p style={{ textAlign: "center", marginTop: "1.25rem", fontSize: 13, color: "#9b8474" }}>
            Již máte účet?{" "}
            <Link href={`/t/${tenantSlug}/login`} style={{ color: "#D97706", fontWeight: 600, textDecoration: "none" }}>Přihlásit se</Link>
          </p>
        </div>
      </div>
    </div>
