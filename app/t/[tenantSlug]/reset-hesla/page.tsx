...
function ResetForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/t/${tenantSlug}/api/auth/reset-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, password }),
      });
      const data = await res.json() as { ok?: boolean; error?: string };
      if (!res.ok) { setError(data.error ?? "Chyba při obnově hesla."); return; }
      setDone(true);
      setTimeout(() => router.push(`/t/${tenantSlug}/login`), 3000);
    } catch {
  ...
  if (!token) {
    return (
      <div style={{ textAlign: "center", display: "flex", flexDirection: "column", gap: 12, alignItems: "center" }}>
        ...
        <Link href={`/t/${tenantSlug}/zapomenute-heslo`} style={{ color: "#D97706", fontWeight: 600, fontSize: 13, textDecoration: "none" }}>
  ...
  return done ? (
    ...
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <div>
        ...
        <input autoComplete="new-password" autoFocus className="auth-input" minLength={6} onChange={(e) => setPassword(e.target.value)} placeholder="Alespoň 6 znaků" required type="password" value={password} />
      </div>
      <div>
        <label className="auth-label">Nové heslo znovu</label>
        <input autoComplete="new-password" className={`auth-input${passwordMismatch ? " auth-input--error" : ""}`} onChange={(e) => setPasswordConfirm(e.target.value)} placeholder="Zopakujte heslo" required type="password" value={passwordConfirm} />
      ...
      <button className="auth-btn" disabled={loading || passwordMismatch} type="submit">
