...
export function createTenantPasswordResetToken(tenantSlug: string, userId: number): string {
  const db = getTenantDb(tenantSlug);
  const token = crypto.randomBytes(32).toString("hex");
...
export function validateTenantPasswordResetToken(tenantSlug: string, token: string): number | null {
  const db = getTenantDb(tenantSlug);
  const row = db
    .prepare(
