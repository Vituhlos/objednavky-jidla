# Oficiální AGENTS.md — @gravity-ui/uikit

Zdroj: https://github.com/gravity-ui/uikit/blob/main/AGENTS.md  
Synchronizuj: `.\scripts\sync-gravity-official-docs.ps1`
...
- **Components**: Design system components are in `uikit/src/components/`, each in its own folder `uikit/src/components/<Name>/` with implementation (`.tsx`) and styles (`.scss`).
- **Stories**: Per-component Storybook files are in `uikit/src/components/<Name>/__stories__/`, and higher-level stories (branding, colors, typography) are in `uikit/src/stories/`.
