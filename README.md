...
### Povinné env proměnné (Auth.js)

Před spuštěním vygeneruj tajný klíč:

```bash
openssl rand -base64 32
```

Bez `AUTH_SECRET` kontejner nastartuje, ale přihlašování selže se silent chybou — uživatel zadá správné heslo, je přesměrován na `/`, ale zůstane nepřihlášený.

| Proměnná | Popis |
|---|---|
| `AUTH_SECRET` | Tajný klíč pro šifrování JWT tokenů. Stabilní napříč restarty — při změně se odhlásí všichni uživatelé. |
| `AUTH_URL` | Veřejná URL aplikace, např. `https://kantyna.firma.local`. Nutné pro OAuth callback URL. |
| `AUTH_TRUST_HOST` | Nastav na `true` pokud aplikace běží za reverzní proxy (nginx, Traefik, Unraid). |
...
services:
  kantyna:
    ...
    volumes:
      - ./data:/app/data
    environment:
      AUTH_SECRET: "sem-vloz-vystup-openssl-rand-base64-32"
      AUTH_URL: https://kantyna.firma.local
...
| Proměnná | Popis |
|---|---|
| `AUTH_SECRET` | **Povinné.** Tajný klíč pro JWT. Generuj: `openssl rand -base64 32` |
| `AUTH_URL` | **Povinné.** Veřejná URL aplikace, např. `https://kantyna.firma.local` |
| `AUTH_TRUST_HOST` | **Povinné za proxy.** Nastav na `true` při běhu za nginx/Traefik/Unraid proxy |
...
| `GOOGLE_CLIENT_ID` | Google OAuth Client ID (alternativně přes UI) | — |
| `GOOGLE_CLIENT_SECRET` | Google OAuth Client Secret (alternativně přes UI) | — |
| `ADMIN_EMAILS` | Čárkou oddělené e-maily automaticky povýšené na admin (alternativně přes UI) | — |
| `DB_PATH` | Cesta k SQLite souboru | `/app/data/stros.db` |

> Většinu nastavení lze měnit přímo v aplikaci přes `/nastaveni`. Hodnoty uložené v aplikaci mají přednost před env proměnnými. Výjimka: `AUTH_SECRET`, `AUTH_URL` a `AUTH_TRUST_HOST` musí být nastavené jako env proměnné — aplikace k nim nemá přístup při inicializaci JWT middleware.
